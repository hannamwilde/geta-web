...
function UpcomingCard({ item, registerLabel }: { item: EventItem; registerLabel: string }) {
  const imgUrl = item.image?.asset ? urlFor(item.image).width(900).height(600).url() : null

  return (
    <article className={`${styles.upcomingCard}${imgUrl ? ` ${styles.upcomingCardWithImage}` : ''}`}>
      {imgUrl && (
        <div className={styles.cardImageWrap}>
          <img src={imgUrl} alt={item.image?.alt || item.title} className={styles.cardImage} />
        </div>
      )}
      <div className={styles.cardContent}>
        ...
        {item.registrationUrl && (
          <a href={item.registrationUrl} className={styles.registerBtn} target="_blank" rel="noreferrer">
            {registerLabel}
            <IconArrowRight />
          </a>
        )}
...
function PastCard({ item }: { item: EventItem }) {
  const imgUrl = item.image?.asset ? urlFor(item.image).width(600).height(400).url() : null

  return (
    <article className={styles.pastCard}>
      {imgUrl && (
        <div className={item.eventType === 'webinar' ? styles.webinarThumb : styles.pastImageWrap}>
          <img src={imgUrl} alt={item.image?.alt || item.title} className={styles.pastImage} />
...
function EventSection({ title, sub, items, registerLabel }: {
  title: string
  sub?: string
  items: EventItem[]
  registerLabel: string
    ...
    <section className={styles.evSection}>
      <div className="container">
        ...
        <div className={styles.upcomingList}>
          {items.map(item => <UpcomingCard key={item._id} item={item} registerLabel={registerLabel} />)}
        </div>
      </div>
    </section>
  )
...
function PastSection({ title, sub, items }: {
  title: string
  sub?: string
  items: EventItem[]
}) {
    ...
    <section className={styles.evSection}>
      <div className="container">
        ...
        <div className={styles.pastGrid}>
          {items.map(item => <PastCard key={item._id} item={item} />)}
        </div>
      </div>
    </section>
  )
...
export default async function Events({ block, upcoming, past }: Props) {
  ...
  const registerLabel = block.registerLabel || t.events.register

  return (
    <div className={styles.evPage}>
      <EventSection
        title={block.upcomingLabel || t.events.upcomingEvents}
        sub={block.upcomingSub}
        items={upcomingEvents}
        registerLabel={registerLabel}
      />
      <EventSection
        title={block.upcomingWebinarLabel || t.events.upcomingWebinars}
        items={upcomingWebinars}
        registerLabel={registerLabel}
      />
      <PastSection
        title={block.pastLabel || t.events.pastEvents}
        sub={block.pastSub}
        items={pastEvents}
      />
      <PastSection
        title={block.pastWebinarLabel || t.events.pastWebinars}
        items={pastWebinars}
      />
    </div>
  )
