...
export default function ListBlock({ block }: Props) {
  ...
  const subheadlineStyle: React.CSSProperties = {
    ...
    ...(block.subheadlineFontSize ? { fontSize: block.subheadlineFontSize + 'px' } : {}),
  }

  return (
    <section className={styles.section} data-item-style={block.itemStyle || 'large'} style={sectionStyle}>
      <div className={styles.container}>
        {(block.eyebrow || block.headline || block.subheadline) && (
          <div className={styles.header}>
            ...
            {block.subheadline && (
              <p className={`${styles.subheadline}${block.subheadlineDivider ? ' ' + styles.subheadlineDivider : ''}`} style={subheadlineStyle}>
                {block.subheadline}
              </p>
            )}
          </div>
        ...
        <div className={styles.grid}>
          {(block.items || []).map((item, i) => {
            const itemImageUrl = item.visualType === 'image' && item.image?.asset
            ...
            const inner = (
              <>
                {item.visualType === 'image' && itemImageUrl ? (
                  <span className={styles.itemImage} data-size={item.imageSize || 'large'}>
                ...
                <span className={styles.itemText}>
                  {item.title && (
                    <h3 className={styles.itemTitle} style={item.titleFontSize ? { fontSize: item.titleFontSize + 'px' } : {}}>
                      {item.title}
                    </h3>
                  )}
            ...
            return item.href
              ? <a key={item._key || i} href={normalizeHref(item.href)} className={styles.item} style={cardStyle}>{inner}</a>
              : <div key={item._key || i} className={styles.item} style={cardStyle}>{inner}</div>
          })}
        </div>
      </div>
    </section>
