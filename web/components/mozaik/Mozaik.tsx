...
export default function Mozaik({ block }: Props) {
  ...
  const angleOf = (i: number) => (-90 + i * (360 / Math.max(nodes.length, 1))) * Math.PI / 180
  const nodePos = (i: number) => ({ x: CX + RX * Math.cos(angleOf(i)), y: CY + RY * Math.sin(angleOf(i)) })
      ...
      <div className={styles.track} ref={trackRef}>
        <div className={styles.stage}>
          ...
          <div className={styles.bg} aria-hidden>
            ...
            <div className={styles.glow} style={mounted ? { transform: `translate(-50%, -50%) scale(${glowScale})`, opacity: 0.4 + p * 0.35 } : undefined} />
          </div>

          {/* Header — fades out as scroll begins */}
          <div
            className={styles.header}
            style={mounted ? { transform: `translateY(${introY}px)` } : undefined}
          >
            ...
            <h2 className={styles.headerTitle}>
              <em>
                <img className={styles.word} src={wordSrc} alt="Mozaik" /> är här,
              </em>
              <br />{'AI-driven digital handel'}
            </h2>
          ...
          <div className={styles.inner}>
            <div className={styles.orbitCol}>
              ...
              <div className={styles.mobileGrid} aria-hidden>
                {nodes.map(n => {
                  const logoUrl = n.logo?.asset ? urlFor(n.logo).width(40).height(40).url() : null
                  return (
                    <div key={n._key} className={`${styles.card} ${styles.cardStatic} ${styles.cardOn}`}>
                      ...
                      <div className={styles.cardBody}>
                        ...
                        {n.description && <div className={styles.cardBlurb}>{n.description}</div>}
                      </div>
                    </div>
                  )
                })}
              ...
              {/* Desktop orbit */}
              <div className={styles.orbit} style={mounted ? { transform: `scale(${ringScale})` } : undefined}>

                {/* Ellipse rings */}
                <svg className={styles.rings} viewBox="0 0 1000 1000" aria-hidden
                  style={mounted ? { transform: `rotate(${ringRot}deg) scale(${ringExtraScale})`, opacity: ringOutro } : undefined}>
                  <defs>
                  ...
                  <ellipse cx="500" cy="500" rx="405" ry="388" className={styles.ring} />
                  <ellipse cx="500" cy="500" rx="300" ry="288" className={`${styles.ring} ${styles.ringMid}`} />
                  <ellipse cx="500" cy="500" rx="195" ry="188" className={`${styles.ring} ${styles.ringInner}`} />
                </svg>

                {/* Connection lines */}
                <svg className={styles.lines} viewBox="0 0 1000 1000" aria-hidden>
                  {mounted && nodes.map((n, i) => {
                    ...
                    return (
                      <line key={n._key} x1={CX} y1={CY} x2={x} y2={y}
                ...
                {/* Node dots */}
                <svg className={styles.dots} viewBox="0 0 1000 1000" aria-hidden>
                  {mounted && nodes.map((n, i) => {
                    const r = revealOf(i)
                    ...
                    const x = CX + (full.x - CX) * f
                    const y = CY + (full.y - CY) * f
                    return <circle key={n._key} cx={x} cy={y} r="5" className={styles.nodeDot} style={{ opacity: r * outroFade }} />
                  })}
                </svg>
                ...
                <div
                  ...
                  style={mounted ? { transform: `translate(-50%, -50%) scale(${hubScale * hubShrink})`, opacity: hubCoreOpacity } : { transform: 'translate(-50%, -50%)', opacity: 0 }}
                >
                  <div className={styles.hubGlow} />
                  <div className={styles.hubDisc}>
                    <img className={styles.mark} src={markSrc} alt="" aria-hidden />
                    <img className={styles.hubName} src={hubNameSrc} alt="Mozaik" />
                ...
                {mounted && nodes.map((n, i) => {
                  ...
                  const side = cos < -0.2 ? styles.cardL : cos > 0.2 ? styles.cardR : styles.cardC
                  const logoUrl = n.logo?.asset ? urlFor(n.logo).width(40).height(40).url() : null
                  return (
                    <div
                      key={n._key}
              ...
              <a
                ...
                } : { opacity: 0, pointerEvents: 'none', transform: 'translate(-50%, -50%) scale(0.5)' }}
              >
